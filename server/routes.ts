    ...
    try {
      ...
      if (item.userId !== req.user!.id) {
        return res
          .status(403)
          .json({
            message: "Forbidden: You don't have permission to access this item",
          });
      }
      ...
      res
        .status(500)
        .json({
          message: "Failed to fetch wardrobe item",
          error: (error as Error).message,
        });
    }
  });
    ...
    try {
      ...
      if (validatedData.imageData) {
        try {
          ...
          if (!validatedData.material)
            validatedData.material = analysisResult.material;
          ...
          // Set the imageUrl to be the data URL for display
          validatedData.imageUrl = `data:image/jpeg;base64,${validatedData.imageData}`;

          // Calculate sustainability score based on material
          validatedData.sustainabilityScore =
  ...
  app.get("/api/recommendations", async (req, res) => {
    try {
      ...
      try {
        ...
        console.error(
          "AI recommendation failed, falling back to basic logic:",
          aiError,
        );
      }
      ...
      if (weatherData.temperature > 25) {
        ...
        const relevantItems = wardrobeItems.filter(
          (item) => item.season === "summer" || item.season === "all-season",
        );

        // Group items by category
        const tops = relevantItems.filter(
          (item) =>
            item.category === "tops" || item.category?.toLowerCase() === "tops",
        );
        const bottoms = relevantItems.filter(
          (item) =>
        ...
        const relevantItems = wardrobeItems.filter(
          (item) => item.season === "winter" || item.season === "all-season",
        );
        ...
        const outerwear = relevantItems.filter(
          (item) =>
        ...
        const accessories = relevantItems.filter(
          (item) =>
  ...
  app.post("/api/weather-preferences", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({
          message: "Failed to save weather preferences",
          error: (error as Error).message,
        });
    }
  });
  ...
  app.get("/api/sustainability/material", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({
          message: "Failed to analyze material sustainability",
          error: (error as Error).message,
        });
    }
  });
