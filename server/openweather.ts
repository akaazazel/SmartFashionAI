...
export async function getWeatherData(location: string): Promise<WeatherData> {
  try {
    const apiKey = process.env.OPENWEATHER_API_KEY || "dummy-key";
    const url = `https://api.openweathermap.org/data/2.5/weather?q=${location}&appid=${apiKey}&units=metric`;
    
    const response = await fetch(url);
    if (!response.ok) {
      throw new Error(`OpenWeather API error: ${response.statusText} (${response.status})`);
    }
    ...
    throw new Error('Weather data unavailable. Please check your API key or try again later.');
  }
}

export function getWeatherIconClass(main: string, timeOfDay: 'day' | 'night'): string {
    ...
    try {
      ...
      const results = Object.values(dailyForecasts).map((day: any) => {
        ...
        return {
          ...
          main: mostFrequent,
          description: day.descriptions.find((desc: string) => 
            desc.includes(mostFrequent.toLowerCase())
          ) || day.descriptions[0],
          icon: day.icons.find((icon: string) => 
      ...
      console.error('Failed to fetch forecast data from OpenWeather API:', error);
      reject(error);
    }
  });
}